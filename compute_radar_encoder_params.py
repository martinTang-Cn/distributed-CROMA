"""
Compute the parameter size of the CROMA optical_encoder and radar_encoder in MB.

- optical_encoder is a ViT (BaseTransformer) with encoder_layers layers.
- radar_encoder is a ViT (BaseTransformer) with encoder_layers // 2 layers.
"""
...
def build_encoders(
    ...
    """构建 CROMA 并返回 (optical_encoder, radar_encoder)。"""
    ...
    return croma.optical_encoder, croma.radar_encoder
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="计算 CROMA radar_encoder 的参数大小 (MB)"
    )
    ...
    print("=" * 56)
    print("optical/radar_encoder 参数规模计算")
    print("=" * 56)
    ...
    optical_encoder, radar_encoder = build_encoders(
