...
class SplitLearningTrainer:
    ...
    def train_split_step(
        self,
        optical_imgs: torch.Tensor,
        radar_imgs: torch.Tensor,
        labels: torch.Tensor,
        optical_encoder_optimizer: torch.optim.Optimizer,
        radar_encoder_optimizer: torch.optim.Optimizer,
        optical_head_optimizer: torch.optim.Optimizer,
        radar_head_optimizer: torch.optim.Optimizer,
        ...
        """
        执行一次 Split Learning 训练步骤（前 10% 样本）

        - 服务器监督损失: 更新 ground_server
        - 服务器损失反传: 仅更新 client encoder
        - 蒸馏损失: 仅更新 client seg_head
        """
        ...
        if enable_distill:
            optical_student_logits = self.optical_client.predict(
                optical_encodings.detach(), output_size=(H, W)
            )
            radar_student_logits = self.radar_client.predict(
                radar_encodings.detach(), output_size=(H, W)
            )
        ...
        # ========== 阶段 2: 激活值传输到服务器（模拟通信） ==========
        self.stats.add_forward(
            optical_encodings.detach(),
            radar_encodings.detach(),
            optical_pred=optical_pred_tx,
            radar_pred=radar_pred_tx,
        )

        # ========== 阶段 3: 服务器前向传播 + 损失计算 ==========
        server_optimizer.zero_grad(set_to_none=True)
        optical_encoder_optimizer.zero_grad(set_to_none=True)
        radar_encoder_optimizer.zero_grad(set_to_none=True)
        ...
        if max_grad_norm > 0:
            ...
            torch.nn.utils.clip_grad_norm_(self.optical_client.encoder.parameters(), max_grad_norm)
            torch.nn.utils.clip_grad_norm_(self.radar_client.encoder.parameters(), max_grad_norm)
        server_optimizer.step()
        optical_encoder_optimizer.step()
        radar_encoder_optimizer.step()
        ...
        if enable_distill:
            ...
            radar_kd_loss = self._distill_and_update(
                ...
                optimizer=radar_head_optimizer,
                temperature=distill_temperature,
                max_grad_norm=max_grad_norm,
                params_to_clip=self.radar_client.seg_head.parameters(),
            )
    ...
    def train_client_step(
        ...
        """
        客户端单独训练（后 90% 样本）: 使用交叉熵训练 client encoder + seg_head。
        """
        optical_imgs = optical_imgs.to(self.device, non_blocking=True)
        radar_imgs = radar_imgs.to(self.device, non_blocking=True)
        labels = labels.to(self.device, non_blocking=True)
        ...
        optical_encoder_optimizer.zero_grad(set_to_none=True)
        radar_encoder_optimizer.zero_grad(set_to_none=True)
        optical_head_optimizer.zero_grad(set_to_none=True)
        radar_head_optimizer.zero_grad(set_to_none=True)

        optical_encodings = self.optical_client(optical_imgs)
        radar_encodings = self.radar_client(radar_imgs)

        optical_logits = self.optical_client.predict(optical_encodings, output_size=(H, W))
        radar_logits = self.radar_client.predict(radar_encodings, output_size=(H, W))

        optical_ce_loss = criterion(optical_logits, labels)
        radar_ce_loss = criterion(radar_logits, labels)
        total_loss = optical_ce_loss + radar_ce_loss

        total_loss.backward()
        if max_grad_norm > 0:
            torch.nn.utils.clip_grad_norm_(self.optical_client.parameters(), max_grad_norm)
            torch.nn.utils.clip_grad_norm_(self.radar_client.parameters(), max_grad_norm)

        optical_encoder_optimizer.step()
        radar_encoder_optimizer.step()
        optical_head_optimizer.step()
        radar_head_optimizer.step()

        return total_loss.item(), optical_ce_loss.item(), radar_ce_loss.item()
    ...
    def _distill_and_update(
        ...
        temperature: float = 1.0,
        max_grad_norm: float = 0.0,
        params_to_clip: Optional[Iterable[nn.Parameter]] = None,
    ) -> float:
        """使用 student logits 与 teacher/peer logits 计算 KL 蒸馏并更新参数，返回标量 loss 值。"""
        optimizer.zero_grad(set_to_none=True)
        student_log_probs = F.log_softmax(student_logits / temperature, dim=1)
        ...
        kd_loss.backward()
        if max_grad_norm > 0:
            params = list(params_to_clip) if params_to_clip is not None else [
...
def train_one_epoch(
    ...
    total_optical_ce_loss = 0.0
    total_radar_ce_loss = 0.0
    num_pixels = 0
    trainer.stats.reset()

    split_steps = max(1, int(len(train_loader) * 0.1))

    for step, (optical, sar, labels) in enumerate(train_loader):
        if step < split_steps:
            loss, server_ce_loss, optical_kd_loss, radar_kd_loss = trainer.train_split_step(
                optical_imgs=optical,
                radar_imgs=sar,
                labels=labels,
                optical_encoder_optimizer=optical_encoder_optimizer,
                radar_encoder_optimizer=radar_encoder_optimizer,
                optical_head_optimizer=optical_head_optimizer,
                radar_head_optimizer=radar_head_optimizer,
            ...
            loss, optical_ce_loss, radar_ce_loss = trainer.train_client_step(
                optical_imgs=optical,
                radar_imgs=sar,
                labels=labels,
                optical_encoder_optimizer=optical_encoder_optimizer,
        ...
        total_optical_ce_loss += optical_ce_loss * batch_pixels
        total_radar_ce_loss += radar_ce_loss * batch_pixels
        ...
        if (step + 1) % args.log_interval == 0:
            ...
            if mode == "split":
                ...
                print(
                    f"[{elapsed_str}] Epoch {epoch} | Step {step+1}/{len(train_loader)} | "
                    f"Mode: local | Loss: {loss:.4f} | "
                    f"optical_ce_loss: {optical_ce_loss:.4f} | "
                    f"radar_ce_loss: {radar_ce_loss:.4f}"
                )
    ...
    avg_optical_ce_loss = total_optical_ce_loss / max(1, num_pixels)
    avg_radar_ce_loss = total_radar_ce_loss / max(1, num_pixels)
    ...
    print(
        ...
        f"avg_optical_ce_loss: {avg_optical_ce_loss:.4f} | "
        f"avg_radar_ce_loss: {avg_radar_ce_loss:.4f}"
    )
    ...
    torch.save(
        {
            ...
            "optical_encoder_optimizer_state_dict": optical_encoder_optimizer.state_dict(),
            "radar_encoder_optimizer_state_dict": radar_encoder_optimizer.state_dict(),
            "optical_head_optimizer_state_dict": optical_head_optimizer.state_dict(),
            "radar_head_optimizer_state_dict": radar_head_optimizer.state_dict(),
...
def main():
    ...
    # 为每个组件创建独立优化器（encoder 与 seg_head 分离）
    optical_encoder_optimizer = torch.optim.AdamW(
        optical_client.encoder.parameters(), lr=args.lr_optical, weight_decay=args.weight_decay
    )
    optical_head_optimizer = torch.optim.AdamW(
        optical_client.seg_head.parameters(), lr=args.lr_optical, weight_decay=args.weight_decay
    )
    radar_encoder_optimizer = torch.optim.AdamW(
        radar_client.encoder.parameters(), lr=args.lr_radar, weight_decay=args.weight_decay
    )
    radar_head_optimizer = torch.optim.AdamW(
        radar_client.seg_head.parameters(), lr=args.lr_radar, weight_decay=args.weight_decay
    )
    ...
    optical_encoder_scheduler = torch.optim.lr_scheduler.StepLR(
        optical_encoder_optimizer, step_size=args.lr_step_size, gamma=0.5
    )
    optical_head_scheduler = torch.optim.lr_scheduler.StepLR(
        optical_head_optimizer, step_size=args.lr_step_size, gamma=0.5
    )
    radar_encoder_scheduler = torch.optim.lr_scheduler.StepLR(
        radar_encoder_optimizer, step_size=args.lr_step_size, gamma=0.5
    )
    radar_head_scheduler = torch.optim.lr_scheduler.StepLR(
        radar_head_optimizer, step_size=args.lr_step_size, gamma=0.5
    )
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, epoch_comm_bytes = train_one_epoch(
            ...
            optical_encoder_optimizer, radar_encoder_optimizer,
            optical_head_optimizer, radar_head_optimizer,
            server_optimizer,
        ...
        optical_encoder_scheduler.step()
        optical_head_scheduler.step()
        radar_encoder_scheduler.step()
        radar_head_scheduler.step()
